...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val closestMap = points.par.groupBy(findClosest(_, means))  // find closest mean for each point
    means.par.map(mean => mean -> closestMap.getOrElse(mean, GenSeq())).toMap  // builds the map
  }
