...
object LineOfSight {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def recurseCalc(start: Int, end: Int, maximum:Float): Unit = {
      if(start < end){
        output(start) = List(input(start) / start, maximum).max
        recurseCalc(start+1, end, output(start))
      }
    }

    recurseCalc(from, until, startingAngle)
